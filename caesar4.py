...
if x==1 :
    ...
    for i in range(len(list1)):
        if ord(list1[i])+int(n) >=32 and ord(list1[i])+int(n) <=126:
            re_list1[i] = chr(ord(list1[i]) + int(n))
        if ord(list1[i])+int(n) > 126:
            re_list1[i] = chr(ord(list1[i]) +int(n) - 94)
...
#加密文件
    ...
    re_file2 = []
    while file2 != "":
        if ord(file2[i])+int(n) >=32 and ord(file2[i])+int(n) <=126:
            re_file2.append(chr(ord(file2[i]) + int(n)))
        if ord(file2[i])+int(n) > 126:
            re_file2.append(chr(ord(file2[i]) +int(n) - 94))
        file2 =file1.readline(1)
    file1.close()
...
#解密文字
    ...
    for i in range(len(list3)):
        if ord(list3[i])+int(n) >=32 and ord(list3[i])+int(n) <=126:
            re_list3[i] = chr(ord(list3[i]) - int(n))
        if ord(list3[i])+int(n) > 126:
            re_list3[i] = chr(ord(list3[i]) +int(n) - 94)
